 ...
 * }
*/
function makePersonObject(id,fullname,email) {
  const person = {
    id,
    name: fullname,
    email
...
function getLastCarInfo(inventory) {
  const iLength = inventory.length -1;
  return 'This is a ' +inventory[iLength].car_make + inventory[iLength].car_model;
}
...
function sortCarInventory(inventory) {
  return inventory.sort(function (model1,model2){
    let a = model1.car_model.toUpperCase();
    let b = model2.car_model.toUpperCase();
...
function getModelYears(inventory) {
  ...
  for (let item in inventory) {
    years.push(inventory[item].car_year);
  }
  return years;
  }
...
function getOlderCars(inventory, maxYear) {
  ...
  for(let item in inventory) {
    if(inventory[item].car_year <= maxYear) {
      oldCars.push(inventory[item]);
    }
  }
  return oldCars;
  /* code here */
...
function getGermanCars(inventory) {
  ...
  for (let item in inventory) {
    if(inventory[item].car_make === 'Audi' ||
      inventory[item].car_make === 'Mercedes-Benz' ||
      inventory[item].car_make === 'Volkswagen' ||
      inventory[item].car_make === 'BMW'){
        germanCars.push(inventory[item]);
      }
  }
  return germanCars;
  /* code here */
